...
beforeEach(() => {
  ...
  vi.mocked(apiRequest).mockReset().mockImplementation(async (path) => {
    ...
    if (path.endsWith("/consents/active")) return { id: "consent-1" } as never;
...
describe("analizador emoción → recomendación → postura", () => {
  ...
  it("explica la falta de consentimiento antes de abrir la cámara", async () => {
    ...
    vi.mocked(apiRequest).mockImplementation(async (path) => path === "/students" ? [{ id: "student-1" }] as never : null as never);
    page();
    await userEvent.click(screen.getByRole("button", { name: "Reconocer mi expresión" }));
    expect(await screen.findByRole("alert")).toHaveTextContent("No hay consentimiento activo");
