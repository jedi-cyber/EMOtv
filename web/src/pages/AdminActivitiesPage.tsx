...
export function AdminActivitiesPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (normalized.steps.some((step) => !step.instruction.trim() || step.duration_seconds <= 0)) {
      setError("Cada paso necesita una instrucción y duración positivas."); return;
    }
  ...
  return <section><PageHeader section="Administración" title="Administrar actividades" description="Crea y actualiza las actividades corporales del catálogo local." />
    ...
    <form className="editor card" onSubmit={save}><div className="form-grid"><label>ID<input required maxLength={128} disabled={editing} value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value.toLowerCase() })} /></label><label>Nombre<input required maxLength={200} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></label><label className="wide">Descripción<input required maxLength={1000} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><label>Postura inicial<select value={form.required_posture} onChange={(e) => setForm({ ...form, required_posture: e.target.value })}><option value="arms_up">Brazos arriba</option><option value="arms_open">Brazos abiertos</option><option value="hands_on_hips">Manos en las caderas</option><option value="arms_forward">Brazos al frente</option><option value="squat">Sentadilla</option></select></label><label>Duración (s)<input type="number" min="0.1" step="0.1" required value={form.duration_seconds} onChange={(e) => setForm({ ...form, duration_seconds: Number(e.target.value) })} /></label><label>Repeticiones<input type="number" min="1" step="1" required value={form.repetitions} onChange={(e) => setForm({ ...form, repetitions: Number(e.target.value) })} /></label></div>
      <h3>Secuencia de posturas</h3><p className="muted">Sin pasos adicionales, se usará la postura inicial. Puedes añadir varios pasos en orden.</p>
      {form.steps.map((step, index) => <div className="form-grid" key={index}><label>Postura {index + 1}<select value={step.posture} onChange={(e) => setForm({ ...form, steps: form.steps.map((item, i) => i === index ? { ...item, posture: e.target.value } : item) })}><option value="arms_up">Brazos arriba</option><option value="arms_open">Brazos abiertos</option><option value="hands_on_hips">Manos en las caderas</option><option value="arms_forward">Brazos al frente</option><option value="squat">Sentadilla</option></select></label><label>Duración (s)<input type="number" min="0.1" step="0.1" value={step.duration_seconds} onChange={(e) => setForm({ ...form, steps: form.steps.map((item, i) => i === index ? { ...item, duration_seconds: Number(e.target.value) } : item) })} /></label><label>Instrucción<input maxLength={1000} value={step.instruction} onChange={(e) => setForm({ ...form, steps: form.steps.map((item, i) => i === index ? { ...item, instruction: e.target.value } : item) })} /></label><button className="button secondary" type="button" onClick={() => setForm({ ...form, steps: form.steps.filter((_, i) => i !== index) })}>Quitar paso</button></div>)}
      <button className="button secondary" type="button" onClick={() => setForm({ ...form, steps: [...form.steps, { posture: "arms_up", instruction: "Eleva los brazos", duration_seconds: 5 }] })}>Añadir paso</button>
